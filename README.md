...
- `apps/api` — Hono on port **5001** in local dev; Better Auth at `/api/auth/*`; storefront JSON API under `/api` (categories, listings, cart, checkout, orders); static assets at `/static/*` (served from `<repo>/public`).
...
The tests expect the web app at `http://localhost:3000` and API at `http://localhost:5001`. Override with `E2E_BASE_URL` / `E2E_API_URL`.
