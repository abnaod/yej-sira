  ...
  return (
    ...
    >
      <div className="mb-4 md:hidden">
        ...
        <h2 className="mt-1 text-lg font-semibold leading-tight text-foreground" aria-current="step">
          {current.title}
        </h2>
      ...
      <div
        className="mb-4 hidden w-full overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:block"
      >
        <ol className="flex min-w-0 w-full list-none flex-wrap items-start justify-start gap-x-6 gap-y-4 p-0 sm:gap-x-10">
          {STEPS.map((s, i) => {
            ...
            if (i < STEPS.length - 1) {
              return (
                <Fragment key={s.key}>
                  ...
                  <li
                    className="flex min-h-11 shrink-0 items-center self-center px-2 sm:px-3"
                    aria-hidden
                  >
                    <ChevronRight
