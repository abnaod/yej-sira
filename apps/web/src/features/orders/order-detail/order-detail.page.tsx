...
async function initializePayment(orderId: string): Promise<InitializePaymentResponse> {
...
export function OrderDetailPage() {
  ...
  const initializePaymentMutation = useMutation({
    mutationFn: (id: string) => initializePayment(id),
    onSuccess: (data) => {
      window.location.href = data.checkoutUrl;
    },
    onError: () => {
      void queryClient.invalidateQueries({ queryKey: ["orderDetail", locale, orderId] });
    },
  });
        ...
        <div className="flex items-center gap-3">
          ...
          {canRetryPayment && (
            <Button
              ...
              onClick={() => initializePaymentMutation.mutate(orderId)}
            >
